...
Once connected, just start talking — on a first visit (no grounding saved yet)
the companion notices and gently offers to set it up. You can also run the
**`begin`** prompt explicitly: in Claude Desktop, the **+** menu → **joshua421**
→ **begin**.

### What `begin` asks, and why one question is unusual

It establishes your intention, tone, weekly rhythm, church day and quiet time — and
one more: **what helps when you have been away for a while and are coming back.**

That last one sets how the nudge behaves during a quiet stretch. Some people want
to find everything exactly as they left it; others want plenty of room and no
questions asked. Both get the email either way — *presence never depends on how
long you have been gone* — but what it asks of you differs:

| Your answer | Saved as | During a quiet stretch |
| --- | --- | --- |
| a gap is just a busy week | `Orientation: steady` | normal, easing off only after a long silence |
| it helps to know nothing was lost | `Orientation: reassure` | unchanged, and the email says so plainly |
| I want room, and no questions | `Orientation: space` | the day and the door, nothing asked, no reply wanted |
| honestly, both | `Orientation: gentle` | the fewest questions, the most predictable |

It lives in your grounding as a plain line you can edit or delete by hand, and it
is **only ever what you said** — joshua421 never infers anything about you from
how you behave ([ADR 0002](./adr/0002-no-inferred-profile.md),
[ADR 0006](./adr/0006-presence-holds-the-ask-scales.md)). Skip the question and
nothing is guessed; it behaves as `steady`.
...
side entries, your grounding — lands on your **primary** calendar by default.
...
This is the only path that needs `better-sqlite3`, a native module. It is an
**optional** dependency and is loaded on demand, so a fresh install never has to
compile it — if you do have a legacy database and the module is missing, migrate
says so and tells you the one command to run. Your file is never touched either way.

Every reflected day becomes a Marker entry; your grounding becomes its one
entry. It's **safe to re-run** (entries upsert against their day),
and it will **never overwrite** a grounding you've since saved through the
...
| Emails stopped after a quiet stretch | Fixed in the secure-base cadence — older builds backed off to weekly and never recovered | Pull the latest, then `npm run doctor` (it now reports when a nudge last went out) |
| No email on a day the laptop was asleep | The worker had no network | Nothing to do — it retries for ~85s, then skips cleanly and resumes tomorrow. The log says `no network after 3 retries` |
| `doctor` says agents were generated by an older install | The launchd plist points its logs at a path this build no longer writes | `npm run worker:install` (regenerates both agents) |
| `doctor` says the loop is not proven | Pipes are fine; no nudge has actually gone out | `npm run worker morning` sends one now |
...
and grounding all live in *your* calendar. See [design.md](./design.md) for
