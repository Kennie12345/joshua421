...
const workerLog = (job: string) => join(repoRoot, 'logs', `${job}.log`)

/**
 * When a nudge was last actually SENT — read from the worker's own log.
 *
 * The doctor used to check pipes and print "the loop should work end to end"
 * while the loop had, in fact, sent four emails in five weeks. Every pipe was
 * green; the outcome was dead. A green pipe is not a delivered email, so the one
 * check worth having is the outcome itself.
 *
 * Read, never stored: the log is the operational record launchd already keeps.
 * Nothing new lands on this machine.
 */
async function lastNudge(): Promise<{ at: string; job: string } | null> {
  let best: { at: string; job: string } | null = null
  for (const job of ['morning', 'evening']) {
    if (!existsSync(workerLog(job))) continue
    let text: string
    try {
      text = await readFile(workerLog(job), 'utf8')
    } catch {
      continue
    }
    for (const line of text.split('\n')) {
      // `[joshua421] job done:  morning @ 2026-08-21T07:00:00.000Z — sent (normal)`
      const m = line.match(/job done:\s+(\w+) @ (\S+) — sent/)
      if (m && (!best || m[2] > best.at)) best = { at: m[2], job: m[1] }
    }
  }
  return best
}

/** Whole days between an ISO timestamp and now — for "last sent N days ago". */
...
async function doctor(): Promise<never> {
  ...
  // Claude Desktop's config path is macOS-shaped. On any other platform its
  // absence says nothing about the install — the user connects a different client
  // by hand — so it must be a note, never a red cross they cannot clear.
  ...
  if (cfg === 'has') ok('Claude Desktop config includes joshua421')
    ...
    note('Claude Desktop config is macOS-only — connect your MCP client by hand (docs/setup.md → Step 2)')
  } else {
  ...
  if (process.platform === 'darwin') {
    ...
    if (agents) {
      ok('daily nudge agents installed (launchd)')
      // A plist generated by an older install can point its logs somewhere this
      // build never writes — the agents look installed, the log looks empty, and
      // the person debugging "why no email" looks in the wrong file.
      const stale = await Promise.all(
        ['morning', 'evening'].map(async (job) => {
          try {
            return !(await readFile(agentPlist(job), 'utf8')).includes(workerLog(job))
          } catch {
            return false
          }
        }),
      ...
      if (stale.some(Boolean)) {
        note('nudge agents were generated by an older install — re-run `npm run worker:install` to refresh them')
      }
    } else {
      note('nudge agents not installed — `npm run worker:install` runs them in the background (optional)')
    }
  }

  // THE OUTCOME. Everything above proves a pipe; this proves an email.
  const nudge = await lastNudge()
  let loopProven = false
  if (!nudge) {
    note('no nudge has been sent yet — expected on a fresh install; `npm run worker morning` sends one now')
  } else {
    const age = daysAgo(nudge.at)
    const when = age === 0 ? 'today' : age === 1 ? 'yesterday' : `${age} days ago`
    if (age <= 7) {
      ok(`last nudge sent ${when} (${nudge.job}, ${nudge.at.slice(0, 10)})`)
      loopProven = true
    }
    else {
      bad(
        `last nudge sent ${when} (${nudge.at.slice(0, 10)}) — the pipes are fine but the loop is not running. ` +
          'Check `npm run worker:status` and the tail of ./logs/.',
      )
      failures++
    }
  }
  ...
  say(
    failures > 0
      ...
      : loopProven
        ? '\nAll clear — every pipe green, and a nudge has actually gone out this week.'
        : '\nPipes all green. No nudge has gone out yet, so the loop is not proven — `npm run worker morning` sends one now.',
  )
